...
select * from guestbook; --*는 모든 컬럼목록을 확인
...
/* 오라클 자료형 종류)
    ...
    제약조건 종류)
      ...
      오라클 시퀀스 객체의 특징)
       1.번호 발생기 이다. 시퀀스는 정수 숫자형으로 선언되고 기본키 제약조건으로 선언된 컬럼(필드)에 사용된다.
...
/* 시퀀스명.nextval로 다음 시퀀스 번호값을 확인
   as "시퀀스번호값" 쿼리문은 검색되는 임시 컬럼 별칭명을 만들 때 사용한다. 실제 오라클에 해당 컬럼 별칭명이 생기는 것은 아니다.
...
/* DMS조작어 중에서 레코드 저장문인 INSERT 문법구조)
   ...
   insert문에서 컬럼목록을 생략하면 테이블 생성시(create table) 생성되는 컬럼순번대로 자료가 저장되고,전체 컬럼에 데이터가 저장된다.
   즉 전체 컬럼(필드)에 레코드 저장시에는 컬럼목록을 생략한다.
*/
insert into guest values(guno_seq.nextval, '홍길동', '방명록제목01', '방명록내용01', sysdate);
--쿼리문(SQL)에서 문자열은 작은 따옴표('')로 처리하고, SQL문은 자바언어 하고는 다르게 영문대소문자를 구분하지 않는다.

insert into guest values(guno_seq.nextval, '이순신', '방명록제목02', '방명록내용02', sysdate);

/* sysdate는 오라클 날짜함수이다. 
오라클 날짜함수 확인쿼리문 다음과 같다.
...
select * from guestbook order by gno desc;  --큰숫자부터 먼저 정렬된다.

/* 레코드 수정문 문법 형식)
    update 테이블명 set 컬럼명=수정할 값, 필드명=변경할값 ... where 조건식;
*/
...
/* 레코드 삭제문 형식문법)
   delete from 테이블명; 
    where 조건식이 없어면 전체 레코드행이 삭제된다. 주의 요망.
...
/*
    문제) 1. 시퀀스를 사용하지 않고 다음과 같은 레코드를 저장해 보자.
            21,'을지문덕','제목21','내용21',sysdate
       
           2. 21번을 기준으로 전체 컬럼의 한 행의 레코드를 검색하는 쿼리문을 만들어 보자.
           
           3. 21번을 기준으로 이름을 '강감찬', 제목을 '수정제목21', 내용을 '수정내용21' 레코드로 변경해 보자.           
*/
...
--2번 답
    select * from guest where gno=21;  -- *는 전체컬럼을 뜻한다
    
--3번 답
   update guest set gname='강감찬', gtitle = '수정제목21', gcont='수정내용21' where gno=21;
    
  select gname,gtitle,gcont from guest where gno=21;
