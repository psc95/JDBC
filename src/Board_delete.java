...
/* 게시판 번호값을 입력받아서 오라클 DB에 해당 번호값이 있으면 삭제시키고 없으면 유효성검증 메시지를 출력한다.*/
public class Board_delete {
	public static void main(String[] args) {
		...
		BoardVO dbNo = bdao.getFindNo(bno);//오라클 DB로 부터 번호값 검색
		if(dbNo != null) {//오라클에 해당 번호값이 있는 경우
			int re = bdao.delBoard(bno);//번호를 기준으로 삭제
			
			if(re == 1)
				System.out.println("게시물 삭제 성공!");
		}else {
			System.out.println("번호값이 없어서 삭제못합니다.");
		}
	}//main
}//class
