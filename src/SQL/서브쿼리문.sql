/* 단일행 서브쿼리문이란 서브쿼리문 실행 결과 레코드 행 값이 한행으로 검색되는 경우를 말한다.
...
--레코드 3개 저장
 ...
 --emp15 테이블 생성
 create table emp15(
  empno number(38) primary key --사원번호 중복금지 null값 금지 not null은 중복만허용
  ,ename varchar2(50) --사원명
  ,job varchar2(50) --직종
  ,sal number(38) -- 월급
  ,comm int -- 보너스 int뒤에 숫자를 안넣으면 38크기가 들어감
...
commit; 
 ...
 select * from emp15 order by empno; --asc문 생략, 사원번호를 기준으롤 오름차순 정렬
...
--단일행 서브쿼리문으로 급여 평균보다 큰 사원명과 급여를 검색
select ename,sal from emp15 where sal > (select avg(sal) from emp15);
 
/*1. 다중행 서브쿼리는 서브 쿼리문의 실행 결과 레코드 행이 하나 이상인 경우를 말한다.
    연산자를 사용할 수 없다.
    ...
    */
    ...
    --다중행 서브쿼리문에서는 단일행 서브쿼리문 연산자는 사용할 수 없다.위에 코드에서=사용 불가
    
    select ename,sal,deptno from emp15
    where deptno in (select distinct deptno from emp15 where sal >= 1200);
    --in 다중행 서브쿼리문 연산자
    
    /* 문제)20번 부서의 최대급여를 구해보자. */
    select max(sal) as "최대급여" from emp15 group by deptno having deptno = 20;
    ...
    select sal from emp15 where deptno = 20;-- 결과값이 2개가 나오기 때문에 단일행 쿼리문을 사용할수가 없음
    
    select ename,sal,deptno from emp15 where sal > all (select sal from emp15 where deptno=20);
    /* 다중행 서브쿼리문 연산자 종류)
        >ALL 연산자는 20번 부서번호의 급여중에서 최대급여보다 크면 참이 되는 연산이다. 결국 20번 부서의 최대 급여보다
        큰 사원명,급여,부서번호를 검색한다. */
        
    /* 문제) 20번 부서의 최소급여 보다 큰 사원명,급여,부서번호를 검색하는 단일행 서브쿼리문을 만들어 보자.*/
    select ename,sal,deptno from emp15 where sal > all (select min(sal) from emp15 where deptno=20);
    ...
    /* >Any 다중행 서브쿼리문 연산자)
        1.20번 부서의 급여중에서 최소값 보다 크면 참이 되는 연산이다.즉 20번 부서의 급여중에서 최소급여보다 큰
        사원명,급여,부서번호를 검색한다. */
 
    --원본 테이블 구조와 레코드를 복제
    create table emp16 as select * from emp15; --emp15 원본테이블의 테이블구조와 레코드까지 모두 복제한 emp16테이블을 생성
    select * from emp16;
 
    --emp15 원본 테이블의 empno,ename,sal,deptno 컬럼과 레코드만 복제한 emp17테이블을 생성
    ...
    /*문제) 원본 테이블의 구조만 복제하고 레코드는 복제하지 않는 emp19 복제 테이블을 만들어 보자. */
