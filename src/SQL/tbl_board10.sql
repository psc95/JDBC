...
create table tbl_board10(
  ...
  ,bhit number(38) default 0 -- default 0제약조건을 설정하면 해당 컬럼에 굳이 레코드를 저장하지 않아도 기본값 0이 저장됨.
  ,bdate date --글쓴 등록날짜
);

 select * from tbl_board10 order by bno desc; --bno즉 게시판 번호를 기준으로 내리참순 정렬해서 큰숫자번호값 부터 먼저 정렬한다.
