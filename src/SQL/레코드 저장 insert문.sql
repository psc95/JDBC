...
--테이블명 다음에 컬럼명을 명시해서 레코드를 저장
insert into emp26 (LOC,ename,empno) values('서울','홍길동',11);

--전체 컬럼에 레코드 저장시 테이블명 다음에 컬럼 목록을 생략
insert into emp26 values(12,'세종대왕님','경복궁');

/* 문제)emp26 원본테이블 구조만 복제한 emp26_copy테이블을 생성하자.원본테이블의 레코드는 복제 안되어있다.
    그러므로 insert~select 서브쿼리문을 사용해서 원본 테이블의 레코드를 복제 테이블에 저장시켜보자.*/
create table emp26_copy as select * from emp26 where 1=2;--조건식을 거짓으로 해서 테이블 구조만 복제한다.
select * from emp26_copy order by empno asc;
insert into emp26_copy select * from emp26 where empno=12; --where을 넣어서 원하는 레코드1줄 복사
insert into emp26_copy select * from emp26 where LOC='서울';
insert into emp26_copy select * from emp26; --서브쿼리문으로 emp26_copy복제테이블에 레코드를 저장하는 실습
...
/* insert ALL 문이란?
    서브 쿼리문의 결과를 조건없이 여러 테이블에 동시에 저장하는 경우 사용한다.
    */
--조건을 거짓으로 해서 사원번호,사원명,급여 컬럼 구조만 복제한 emp28테이블 생성
create table emp28 as select empno,ename,sal from emp27 where 10 = 0;
select * from emp28;
delete from emp28;
commit;

--조건을 거짓으로 해서 사원번호,사원명,사는 지역 컬럼만 복제한 emp29
create table emp29 as select empno,ename,LOC from emp27 where 8 = 0;
select * from emp29;

/*emp27원본 테이블의 부서번호 20에 해당하는 사원번호,사원명,급여,사는 지역을 검색해서 emp28테이블에는
    사원번호,사원명,급여만 저장하고 emp29테이블에는 사원번호,사원명,사는지역을 동시에 저장한다.*/
    insert ALL
    ...
    */
    --조건식으로 거짓으로 해서 emp27원본 테이블의 empno,ename,deptno 컬럼만 복제한 emp30테이블 생성
    create table emp30 as select empno,ename,deptno from emp27 where 10 = 8;
    select * from emp30;
    
    --where 조건식을 거짓으로 해서 emp27 원본 테이블의 empno,ename,sal 컬럼 구조만 복제한 emp31테이블을 생성
    create table emp31 as select empno,ename,sal from emp27 where 1=0;
    select * from emp31;

    select empno,ename,sal,deptno from emp27;

    insert all
    when sal >= 1000 then --급여가 1000이상인 경우 다중행 저장
    ...
    select * from emp30 order by empno;
